...
- **公開後追加①（2026-07-08, commit d2b5cd2）**: スマホの**二本指ピンチズーム**対応。`src/nav/pinch.ts`(TDD) + InputMapper を pointerId 位置追跡に作り替え（一本指ドラッグを movementX/Y 依存→位置差分にしてタッチでも視点回転が効くよう堅牢化、二本指はピンチ専念、複数指ジェスチャ中はタップ選択抑制、setPointerCapture を try/catch 保護）。ControlHints はタッチ端末で「1本指ドラッグ/2本指ピンチ/タップ」表示に。→ 「ピンチズーム未対応」の deferred は解消。
- **公開後追加②（2026-07-08）**: 恒星間パネルに星の数の説明追加（「まわりの光の点は恒星／天の川には約2000億個」）。
- **公開後追加③＝宇宙論的ズームアウトに作り替え（2026-07-08, commit b4ae88f + universe段）**: **アンドロメダ銀河とクロスフェードを全撤去**（唐突に切り替わる問題）。代わりに天の川からズームアウトで **銀河団→超銀河団→観測可能な宇宙** と大規模構造が現れる連続ズームに。`src/galaxy/cosmicWeb.ts`(node+フィラメント+ボイドの網目・決定論生成/TDD/CosmicWeb クラス) と `src/galaxy/universeHorizon.ts`(CMB風の地平線球殻/TDD/UniverseHorizon クラス) を追加、galaxy シェーダ再利用。フェードは `localGroupFade`(天の川)/`cosmicWebFade`(1.2e10→8e10)/`universeFade`(8e11→3e12)。scaleInfo ステージを `localgroup`→`cluster/supercluster/universe` に。**MAX_VIEW_AU 5e10→6e12** に拡張。銀河団以遠は縮尺バー非表示。220テスト緑。本番で全6段(太陽系→太陽系の外へ→天の川銀河→銀河団→超銀河団→観測可能な宇宙)・console 0 確認。user「一旦これで完成」。
- **デプロイ手順（確立）**: 修正 → `npm run build` → **プレビュー確認は `wrangler pages deploy dist --project-name stellar-voyage --branch preview`（→ https://preview.stellar-voyage.pages.dev・本番不変）** → OK なら **`--branch main` で本番昇格**（同一アーティファクトなら再アップロード0）。
...
- **重要バグ（修正済）**: ①恒星間/銀河で最近傍(焦点)星が星野から消えラベルだけ浮く→fade==0で隠さない
  ...
  ③直径定規がズームイン中ずっと全幅の正体不明線→画面幅超で隠す ④スペクトル分類でシリウス/ベガが「B」
  →色温度式に境界を合わせA型に ⑤遠方惑星/光度/年数の指数表記(2.4e+2等) ⑥縮尺バー「光で0分」。
- **軽微/改善（修正済）**: フェードが軌道リング基準不透明度を潰す・銀河公転円0.5上書き・スライダー対象固着・
  formatManKm「約10000万km」・銀河直径ラベルと radiusAu の26%乖離(radiusAuを実スケール10万/20万光年に統一)・
  星クリック遮断閾値(6.5e9→localGroupFade>=1で見えてる星を押せるように)・パネル残留hide・setFocus再アップロード
  抑制・最遠段のfield描画スキップ・InfoPanel を textContent 化(XSS)。
- **技術的負債（整理済）**: dead module 6件+テスト4件削除(src参照ゼロ確認)・favicon追加で404解消・
  build-exoplanets の CSV をクオート対応化+a<=0除外(再生成し出力同一)・README を現行UXに刷新・
  **SDD台帳 progress.md を git 追跡化**(旧 `*` gitignore でバックアップゼロだった一次ソースを保全)。
- **検証**: 純粋ロジックは TDD(RED→GREEN)、描画/DOM は Playwright E2E(焦点星復活・定規クランプ・秒表示・
  銀河ラベル10万/20万光年・crossfade健全・光速バー動作・console 0エラー)。207テスト緑・tsc・build(502KB)緑。
- **未対応（据置・非ブロッキング）**: git リモート未設定＝バックアップは依然ローカルのみ（GitHub 公開/private の
  判断が user 必要）／ピンチズーム未対応（モバイル実機検証が必要）／HZ 判定の bolometric 補正（K/M型の
  実在+ハビタブル境界が内寄り・精度モデルの判断）／光速バー/直径定規の spec/plan 未整備（実装は handoff/ledger のみ）。

## ✅ 前の完了 — 光速バー（光速パルスを作り替え）（2026-07-07）
