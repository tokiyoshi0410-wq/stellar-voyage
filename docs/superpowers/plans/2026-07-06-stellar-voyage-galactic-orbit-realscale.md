...
コンストラクタ: 天の川の傾きは既存の `this.milkyWay.object.rotation.x = 0.5;` のまま（Euler order は既定 `'XYZ'` を使う）。**⚠️ 実装訂正(d3bdd9b): 当初この plan は `rotation.order = 'YXZ'` を指示していたが誤り。既定 XYZ は `Rx·Ry` ＝ 傾いた法線まわりの面内自転で正しく、YXZ は `Ry·Rx` ＝ world Y まわりで円盤が歳差でぐらつく。`rotation.order` は設定しないこと。**
