...
### Task 2: 描画 `LightPulse.ts`（広がる半透明球）
...
**Interfaces:**
- Produces: `class LightPulse { readonly object: THREE.Mesh; update(radiusAu: number): void; setVisible(v: boolean): void; dispose(): void }`
...
```bash
git add src/edu/LightPulse.ts tests/edu/lightPulseMesh.test.ts
...
**Interfaces:**
- Consumes: Task1 `pulseGrowthAuPerSec`/`pulseLightTimeMin`/`formatPulseTime`/`pulseReached`、Task2 `LightPulse`、Task3 `EmitButton`、Task4 `PulseReadout`、既存 `systemFade`(`./nav/fade`)・`nearestStarsPc`(`./nav/nearestStars`)・`AU_PER_PC`(`./starfield/StarField`)。
...
```typescript
import { LightPulse } from './edu/LightPulse';
...
```typescript
  const lightPulse = new LightPulse();
...
見た目が弱ければ `PULSE_SPEED_FRACTION`（lightPulse.ts）・球の `opacity`/`color`（LightPulse.ts）・ボタン/カウンタの `bottom` 値を実機調整（テストに影響しない見た目パラメータ）。
...
- **Type consistency:** `pulseGrowthAuPerSec`/`pulseLightTimeMin`/`formatPulseTime`/`pulseReached`（Task1）と Task5 の呼び出し、`LightPulse.update/setVisible/dispose`（Task2）と Task5、`nearestStarsPc` の返り `{index,distPc}`（既存）と Task5 の `.distPc`/`.index` 参照が一致。
